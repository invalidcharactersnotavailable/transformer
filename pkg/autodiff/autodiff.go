...
func NewRandomMatrix(rows, cols int) *Matrix {
	...
	for i := 0; i < rows; i++ {
		for j := 0; j < cols; j++ {
			matrix.Data[i][j] = (float64(i+j) / float64(rows+cols)) * 0.1
		}
	}
	return matrix
}
...
// TensorMatMul performs matrix multiplication with gradient tracking
...
// TensorAdd performs element-wise addition with gradient tracking
...
// TensorReLU applies the ReLU activation function with gradient tracking
...
// TensorGELU applies the GELU activation function with gradient tracking
...
// TensorSoftmax applies the softmax function with gradient tracking
...
// TensorCrossEntropyLoss computes the cross-entropy loss with gradient tracking
...
// TensorMSELoss computes the mean squared error loss with gradient tracking
