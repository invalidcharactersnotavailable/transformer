...
import (
	"math"
	"fmt" // Added for fmt.Errorf and fmt.Printf
)
...
type Expert struct {
	...
	W2         *Tensor
	B2         *Tensor
	Activation func(*Tensor) (*Tensor, error) // Now returns error
	Graph      *ComputationGraph // So expert ops can be part of the graph
}
...
type Router struct {
	Weights *Tensor // modelDim x numExperts
	Bias    *Tensor // 1 x numExperts
...
func NewZerosTensorFallback(rows, cols int, config *TensorConfig) *Tensor {
	tensor, err := NewZerosTensor(config, rows, cols) // Assuming NewZerosTensor signature is (config, shape...)
	if err != nil {
